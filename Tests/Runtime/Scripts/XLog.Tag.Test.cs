...
internal class TestXLogTag
{
    ...
    public void Basic()
    {
        ...
        Assert.AreEqual("value1", tag.Get("key1"),
            "期望 key1 的值为 'value1'");
        Assert.AreEqual("value2", tag.Get("key2"),
            "期望 key2 的值为 'value2'");
        Assert.AreEqual("", tag.Get("nonexistent"),
            "期望不存在的键返回空字符串");
        ...
        Assert.AreEqual("value1", data["key1"],
            "期望字典中 key1 的值为 'value1'");
        Assert.AreEqual("value2", data["key2"],
            "期望字典中 key2 的值为 'value2'");

        // 测试日志级别
        tag.Level = XLog.LevelType.Debug;
        Assert.AreEqual(XLog.LevelType.Debug, tag.Level,
            "期望日志级别设置为 Debug");

        tag.Level = XLog.LevelType.Info;
        Assert.AreEqual(XLog.LevelType.Info, tag.Level,
            "期望日志级别设置为 Info");

        // 测试克隆功能
        var clonedTag = tag.Clone();
        Assert.AreEqual("value1", clonedTag.Get("key1"),
            "期望克隆标签包含原始标签的 key1 值");
        Assert.AreEqual("value2", clonedTag.Get("key2"),
            "期望克隆标签包含原始标签的 key2 值");
        Assert.AreEqual(tag.Text, clonedTag.Text,
            "期望克隆标签的文本表示与原始标签相同");

        clonedTag.Set("key3", "value3");
        Assert.AreEqual("", tag.Get("key3"),
            "期望原始标签不受克隆标签修改的影响");
        Assert.AreEqual("value3", clonedTag.Get("key3"),
            "期望克隆标签可以独立设置新的键值对");

        // 测试空标签
        var emptyTag = XLog.GetTag();
        Assert.AreEqual("", emptyTag.Text,
            "期望空标签的文本表示为空字符串");
        Assert.AreEqual(0, emptyTag.Data.Count,
            "期望空标签的字典为空");
