...
internal class TestXPool
{
    ...
    public void SObject()
    {
        ...
        Assert.That(XPool.SObject<List<int>>.pools, Has.Count.LessThanOrEqualTo(XPool.SObject<List<int>>.PoolMax),
            "对象池数量不应超过设定的上限值");
    ...
    public IEnumerator GObject()
    {
        ...
        // 测试Set功能
        Assert.IsTrue(XPool.GObject.Set(key, prefab, XPool.GObject.CacheType.Global),
            "设置预制体到全局对象池应成功");
        Assert.IsTrue(XPool.GObject.Has(key),
            "对象池中应能找到已设置的预制体");

        // 测试Get功能
        var obj1 = XPool.GObject.Get(key);
        Assert.IsNotNull(obj1, "从对象池实例化的游戏对象不应为空");
        Assert.AreEqual(prefab.name, obj1.name,
            "实例化的对象名称应与预制体一致");

        // 测试Put功能
        XPool.GObject.Put(obj1);
        yield return null;  // 等待一帧确保Put完成

        // 测试对象复用
        var obj2 = XPool.GObject.Get(key);
        Assert.That(obj2, Is.SameAs(obj1),
            "对象池应返回之前回收的同一个游戏对象实例");

        // 测试Del功能
        Assert.IsTrue(XPool.GObject.Del(key),
            "从对象池中删除预制体应成功");
        Assert.IsFalse(XPool.GObject.Has(key),
            "删除后对象池中不应再存在该预制体");
    ...
    public void SBuffer()
    {
        ...
        Assert.AreEqual(1024, buffer1.Capacity,
            "字节流容量应与请求的大小一致");
        Assert.AreEqual(0, buffer1.Length,
            "新创建的字节流长度应为0");
        Assert.AreEqual(0, buffer1.Position,
            "新创建的字节流位置应为0");

        // 测试写入和Flush
        var testData = new byte[] { 1, 2, 3, 4 };
        buffer1.Writer.Write(testData);
        Assert.AreEqual(4, buffer1.Position,
            "写入数据后流位置应等于写入的数据长度");
        buffer1.Flush();
        Assert.AreEqual(4, buffer1.Length,
            "Flush后流长度应等于最后写入位置");
        Assert.AreEqual(0, buffer1.Position,
            "Flush后流位置应重置为0");
        ...
        Assert.That(buffer2, Is.SameAs(buffer1),
            "缓冲池应返回之前缓存的同一个字节流实例");
        Assert.AreEqual(-1, buffer2.Length,
            "复用的字节流长度应被重置为-1");
        Assert.AreEqual(0, buffer2.Position,
            "复用的字节流位置应被重置为0");

        // 测试获取更大容量的buffer
        var buffer3 = XPool.SBuffer.Get(2048);
        Assert.That(buffer3, Is.Not.SameAs(buffer1),
            "请求更大容量时应创建新的字节流实例");
        Assert.AreEqual(2048, buffer3.Capacity,
            "新字节流容量应与请求的大小一致");
        ...
        Assert.That(newLargeBuffer, Is.Not.SameAs(largeBuffer),
            "超过最大字节限制的缓冲不应被复用");
        ...
        Assert.That(XPool.SBuffer.buffers, Has.Count.LessThanOrEqualTo(XPool.SBuffer.PoolMax),
            "缓冲池中的实例数量不应超过设定的上限值");
